Feature: control multiple openflow switchies using sliceable_switch
  ...
  Scenario: One openflow switch, one slice, three servers, mac binding ( update config )
    Given the following slice records
      | slice_id | description |
      | test1    | slice1      |
    And the following mac binding records
    ...
    Given the following mac binding records
      | slice_id | address           | binding_id |
      | test1    | 00:00:00:00:00:02 | host2      |
      | test1    | 00:00:00:00:00:03 | host3      |
    ...
    When I delete mac binding "host3" from slice "test1"
    And *** sleep 5 ***
    And I reset stats to host1
    And I reset stats to host2
    And I reset stats to host3

Feature: control multiple openflow switchies using sliceable_switch
  ...
  Scenario: One openflow switch, one slice, three servers, port mac binding ( update config )
    Given the following slice records
      | slice_id | description |
      | test1    | slice1      |
    And the following port binding records
    ...
    And the following port and mac binding records
      | slice_id | port_binding_id | address           | binding_id |
      | test1    | host2           | 00:00:00:00:00:02 | host2_mac  |
      | test1    | host3           | 00:00:00:00:00:03 | host3_mac  |
    ...
    When I delete mac binding "host3_mac" on port "host3" from  slice "test1"
    And I delete port binding "host3" from slice "test1"
    And *** sleep 5 ***
    And I reset stats to host1
    And I reset stats to host2
    And I reset stats to host3
